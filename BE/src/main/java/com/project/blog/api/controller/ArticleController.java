...
import com.project.blog.api.model.article.*;
...
import com.project.blog.domain.service.*;
...
public class ArticleController {
    ...
    @GetMapping("/feed")
    @CheckSecurity.Public.canRead
    public ArticleWrapper getFeed(@RequestParam(required = false, defaultValue = DEFAULT_FILTER_LIMIT) int limit,
	    @RequestParam(required = false, defaultValue = DEFAULT_FILTER_OFFSET) int offset) {
	var profile = userService.getCurrentUser().getProfile();
	Pageable pageable = PageRequest.of(offset, limit, DEFAULT_FILTER_SORT);
